...
Сборка не скачивает чужие моды. JEI, Jade, AE2 и Ex Machina (библиотека внутри More Red) присутствуют в игре собственными jar, а код мода
компилируется против заготовок в `provided/`: по одной на чужой тип, только сигнатуры, которые мод
вызывает или реализует. Сами jar для dev-запусков лежат рядом с репозиторием, в `../stands/_mods`.
...
| сигнатуры совпадают с настоящими до байта: jar, собранный против заготовок, равен jar, собранному против настоящих модов | локальный судья до публикации: та же сборка против настоящих jar из `../stands/_mods`, архивы сверяются побайтно |
| стенды (`-Pgt6probes`) читают внутренности модов, поэтому компилируются против настоящих jar из `../stands/_mods` | `build.gradle` |
