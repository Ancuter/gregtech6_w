	...
	public static class WorldGenContainer implements Runnable {
		...
		public void run() {
			if (!mGenNormal.isEmpty()) {
				...
				// Was getBiomeGenForWorldCoords via the removed WorldProvider.worldChunkMgr, with a manual null-fallback to
				// removed biome constants; neo's getBiome never returns null, so per-column surface height feeds the always-3D query.
				...
				// Was a manual precipitationHeightMap reset (a 1.7.10 workaround against snow killing stumps); the field is
				// gone and modern Heightmap recomputes on the fly, apparently making a manual reset unnecessary, though unconfirmed.
				tChunk.markUnsaved();
			}
		}
	}
