...
public class Behavior_Bucket_Simple extends AbstractBehaviorDefault {
	...
	public ItemStack onDispense(MultiItem aItem, BlockSource aSource, ItemStack aStack) {
		...
		Direction aFacing = aSource.state().getValue(DispenserBlock.FACING); // BlockSource is now a record; facing comes from its BlockState instead of the old metadata lookup.
