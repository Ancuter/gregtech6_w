...
/** The 1.7.10 static Enchantments.FORTUNE/LOOTING instances are gone: enchantments are data-driven now and
 *  a Holder<Enchantment> needs a live RegistryAccess unavailable in a static context, so this degrades to an empty array. */
public abstract class ToolStats implements IToolStats {
	...
	public boolean harvestGrass(List<ItemStack> aDrops, ItemStack aStack, Player aPlayer, Block aBlock, long aAvailableDurability, int aX, int aY, int aZ, byte aMetaData, int aFortune, boolean aSilkTouch, BlockDropsEvent aEvent) {
		// Blocks.BUSH is new in 26.1.2 (did not exist in 1.7.10) and is folded into the short-grass branch.
		...
		// Blocks.SHORT/TALL_DRY_GRASS are new in 26.1.2; cutting them yields dry hay directly, with the same
		// single/double-block yield rule as their wet counterparts.
