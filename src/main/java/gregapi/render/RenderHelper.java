...
public class RenderHelper {
	...
	/** Live wrench-hover overlay, 1:1 with 1.7.10's raw GL11 drawing in DrawBlockHighlightEvent: face bases come from
	 *  the same CCL Rotation.sideRotations math, with all 112 line segments transcribed from the original's vertex pairs. */
	...
	public static void drawWrenchOverlay(net.neoforged.neoforge.client.event.ExtractBlockOutlineRenderStateEvent aEvent, byte aConnections, byte aSide) {
		...
		aEvent.addCustomRenderer((aState, aBuffer, aPose, aTranslucent, aLevelState) -> {
			// Draws in the same pass where vanilla draws its own block outline, gated the same way by isTranslucent.
			if (aState.isTranslucent() != aTranslucent) return false;
			try { drawWrenchOverlayNow(aBuffer, aPose, aLevelState, fX, fY, fZ, fConnections, fSide); } catch (Throwable e) {/* cosmetics must not crash the frame */}
			return false; // the vanilla outline is NOT suppressed, since 1.7.10 drew this overlay ON TOP of the selection box
		});
	}
