...
public class BlockMetaType extends BlockBaseMeta implements net.minecraft.world.level.block.SimpleWaterloggedBlock {
	...
	public BlockMetaType(Class<? extends BlockItem> aItemClass, Material aVanillaMaterial, SoundType aSoundType, String aNameInternal, String aDefaultLocalised, OreDictMaterial aMaterial, float aResistanceMultiplier, float aHardnessMultiplier, int aHarvestLevel, int aCount, IIconContainer[] aIcons) {
		...
		// Slabs are built inside this constructor rather than at a normal call site, so nothing else registers them; each unique
		// slab is registered directly here instead, while construction still runs during the unfrozen block registry event.
		{
	...
	// Without this bridge nothing called the body below: 1.7.10's onBlockActivated moved to useItemOn/useWithoutItem, and
	// since the rule lived on this class, not the BlockBase root, placing it here covers BlockStones too, which inherits it.
	...
	protected boolean skipRendering(BlockState aState, BlockState aNeighbor, Direction aDir) {
		...
		if (aSide != mSide && SIDES_VALID[mSide]) {
			...
			// neo's Block has no generic override point for this, so it's reintroduced as a constant method, the same technique used
			// elsewhere; note the contract here is inverted (true means hide, not draw), unlike the 1.7.10 original.
