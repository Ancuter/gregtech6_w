...
public abstract class BlockBaseBars extends BlockBaseSealable implements IRenderedBlock, gregapi.block.IBlockExtendedMetaData {
	...
	/** Asks the already-centralized GT6QuadBuilder resolver for the vanilla iron-bars sprite rather
	 *  than adding a second resolution path; client-only since the atlas exists only there. */
	// CS.CODE_CLIENT can't decide this: both proxies set it, and whichever initializes last wins, silently returning null
	// and leaving particles grey; this checks the real physical side instead, needed since the resolver is client-only.
