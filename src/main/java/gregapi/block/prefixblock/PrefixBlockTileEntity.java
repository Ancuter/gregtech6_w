...
public class PrefixBlockTileEntity extends TileEntityBase01Root implements IRenderedBlockObject, IRenderedBlockObjectSideCheck, ITileEntityScheduledUpdate, ITileEntitySynchronising, ITileEntitySpecificPlacementBehavior {
	...
	@Override public void readFromNBT(CompoundTag aNBT) {super.readFromNBT(aNBT); mMetaData = aNBT.getShort("m").orElse((short)0); if (aNBT.contains("gt.nbt.drop")) mItemNBT = aNBT.getCompoundOrEmpty("gt.nbt.drop"); mTexture = null;/* rebuild the texture after the material loads or syncs */}
