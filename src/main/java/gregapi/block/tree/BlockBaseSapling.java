...
public abstract class BlockBaseSapling extends BlockBaseMeta implements IPlantable, BonemealableBlock, IOxygenReliantBlock, gregapi.render.IRenderedCross {
	...
	@Override public boolean isValidBonemealTarget(net.minecraft.world.level.LevelReader aWorld, BlockPos aPos, BlockState aState) {return T;} // a sapling is always a valid bonemeal target
	...
	public boolean tryGrow(Level aWorld, int aX, int aY, int aZ, Random aRandom) {
		...
		// neo has the same tree-growth veto ability through EventHooks.fireBlockGrowFeature, the path vanilla's own TreeGrower
		// uses; GT6 passes a null feature holder since it grows imperatively, checking only whether the event was cancelled.
