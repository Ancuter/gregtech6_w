...
public class MultiTileEntityRegistry {
	...
	/** The numeric registry key is a locally assigned item id, differing between JVMs and mod sets unlike 1.7.10's stable
	 *  config-based ids, so resolution tries the name first, then the number, then the sole registry if there's only one. */
	...
	/** The registry name is written alongside the number by every caller that saves or sends this identity. */
