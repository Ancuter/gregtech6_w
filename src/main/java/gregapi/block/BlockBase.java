...
public abstract class BlockBase extends Block implements IBlockBase {
	...
	// 1.7.10's engine asked the block directly for its light opacity; neo instead reads dampening from
	// BlockState, so without this bridge GT6's values never reached the engine and used neo's own default instead.
	...
	// Vanilla's Block.getIcon(int,int) was removed along with the whole IIcon atlas, so it is restored
	// locally here, giving the call above and every subclass override one shared point again.
