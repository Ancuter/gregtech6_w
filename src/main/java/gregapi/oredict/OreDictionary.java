...
/** Central replacement for Forge's OreDictionary, which neo removes entirely; GT6 already owns the richer
 *  semantics on top, so only the flat storage and its registration event are reproduced, 1:1 with Forge's hash rule. */
public class OreDictionary {
	...
	/** Reproduces the vanilla dictionary entries Forge itself registered before any mod loaded, or GT6 sees a
	 *  half-empty dictionary; new post-1.7.10 content uses only existing names, and vanilla copper is excluded. */
	public static void initVanillaEntries() {
		...
		// These ore families are split by the engine itself, not by a mod (its own ore tags list every variant);
		// new members are registered under the same name as the first-listed member of that family.
		...
		// Ancient debris is a Nether ore like nether quartz above; its material name is Grego's own "AncientDebris".
		...
		// New post-flattening vanilla storage blocks continue this same list under the same naming rule; the
		// amethyst block is deliberately excluded since its recipe yields four shards, not the nine GT6's gem block expects.
		...
		// New vanilla material items continue this list under the same existing naming rules; vanilla copper is
		// excluded here on purpose since a separate loader owns it and deliberately targets the GT6 ingot instead.
