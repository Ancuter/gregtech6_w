...
public class Loader_Recipes_Replace implements Runnable {
	...
	public static ItemStack getRecipeOutput(ICraftingRecipeGT aRecipe, ItemStack... aStacks) {
		...
		if (!tAny) return null;
		// neo's CraftingInput is immutable (no setItem, no public ctor); items assemble via CraftingInput.of(w,h,List).
		// GT6's own matches/getCraftingResult drive the recipe, not neo's assemble.
