...
public class WorldgenHives extends WorldgenObject {
	...
	public boolean placeHive(MultiTileEntityRegistry aRegistry, int aDimType, WorldGenLevel aWorld, int aX, int aY, int aZ, int aColor, int aSpeciesID, Random aRandom) {
		...
		if (aDimType == DIM_OVERWORLD) {
			...
			aBumbleTag = IItemBumbleBee.Util.getBumbleGenes(WD.envTemp(aWorld, aX, aY, aZ), WD.biome(aWorld, aX, aZ), !!aWorld.dimensionType().hasSkyLight() && WD.precipitationHeight(aWorld, aX, aZ) <= aY + 5, aWorld.getLevel().isBrightOutside(), !aWorld.getLevel().isBrightOutside(), aRandom); // isBrightOutside is Level-only; the worldgen receiver WorldGenLevel reaches the eventual ServerLevel via getLevel().
		}
