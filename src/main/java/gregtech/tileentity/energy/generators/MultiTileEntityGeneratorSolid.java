...
public abstract class MultiTileEntityGeneratorSolid extends TileEntityBase09FacingSingle implements ITileEntityEnergy, ITileEntityRunningActively, IMTE_GetCollisionBoundingBoxFromPool, IMTE_OnEntityCollidedWithBlock, gregapi.block.multitileentity.IMultiTileEntity.IMTE_GetLightValue {
	...
	public void readFromNBT2(CompoundTag aNBT) {
		...
		if (aNBT.contains(NBT_FUELMAP)) {RecipeMap tMapGuard = RecipeMap.RECIPE_MAPS.get(aNBT.getString(NBT_FUELMAP).orElse("")); if (tMapGuard != null) mRecipes = tMapGuard;} /* Don't overwrite the default on a null lookup. */
		...
		if (isServerSide()) {
			...
			if (aStack == null) {
				...
				if (canInsertItem2(0, aStack, SIDE_INSIDE)) {
					slot(0, aStack);
					aPlayer.getInventory().setItem(aPlayer.getInventory().getSelectedSlot(), ST.nn(NI)); // Engine boundary: GT6 null maps to engine EMPTY; setItem(null) on a NonNullList threw, once swallowed silently.
					return T;
				}
